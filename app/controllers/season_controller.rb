...
class SeasonController < ApplicationController
  ...
  def show
    ...
    render :template => 'season/show', :locals => { :episodes => episodes, :id => id, :all => false, :info => info, :watches => watches, :extras => extras }
  end

  def all
    id = params[:series_id]

    watches = Watch.where(user_id: current_user.id, series_id: id).pluck('episode_id')
    info = Series.where(id: id).select('poster_thumb, name').first
    ...
    render :template => 'season/show', :locals => { :episodes => episodes, :id => id, :all => true, :info => info, :watches => watches, :extras => extras }
  end
end
