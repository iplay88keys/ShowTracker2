    ...
    if params[:remote]
      ...
      @results.each do |result|
        if Series.where(id: result.id).first == nil
          ...
          @series = Series.create(id: result.id, name: result.series_name, poster: result.poster ? result.poster : nil, poster_thumb: result.poster ? result.poster.gsub(/banners\//, "banners/_cache/") : nil, banner: result.banner ? result.banner : nil, banner_thumb: result.banner ? result.banner.gsub(/banners\//, "banners/_cache/") : nil, overview: result.overview == nil ? "" : result.overview, status: result.status, last_updated: result.lastupdated.to_i)
        end
      end
    end
